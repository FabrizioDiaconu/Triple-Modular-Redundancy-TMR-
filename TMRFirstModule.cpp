...
#define FIRST_MPU_WARNING_LED_PIN 32
...
MCP2515 firstMCP(CS_PIN);// Creating an instance for the can bus module
...
Receiver secondCAN_data, thirdCAN_data;

Adafruit_MPU6050 firstMPU;// Creating an instance for the mpu6050 sensor

Servo firstServo;// Creating an instance for the servomotor
...
bool firstMPUWorking = false, firstMPUReading = false, warningLed = LOW;
long unsigned int lastRestartingTime = 0;
// Variables made for the fault tolerance system regarding the other ESP32s
long unsigned int secondCANTime = 0, thirdCANTime = 0;
bool secondMPUWorking = false, thirdMPUWorking = false;
...
void setup()
{
    ...
    firstMCP.reset();
    firstMCP.setBitrate(CAN_125KBPS, MCP_8MHZ);
    firstMCP.setNormalMode();
    ...
    Wire.setTimeOut(3); /* Setting 3 ms as a time reference for the mpu I2C communication,
                            if that time passes by and the ESP32 hasn't received new data yet,
                            it will use the data from the other MPUs*/
    // Checking if the MPU6050 sensor is physically connected to the I2C bus
    Wire.beginTransmission(MPU_I2C_ADDR);
    if(Wire.endTransmission() == 0) 
    {
        firstMPUWorking = firstMPU.begin(); /* If the I2C communication is successful, 
                                        try to initialize the sensor library */
    } 
    else 
    {
        firstMPUWorking = false; // If the I2C transmission failed, mark the local MPU as not working
    }
    if(!firstMPUWorking) // Handling the fault tolerance
    {
    ...
    firstServo.attach(SERVO_PIN);
    firstServo.setPeriodHertz(50);

    pinMode(BUTTON_PIN, INPUT);
    pinMode(ONOFF_LED_PIN, OUTPUT);
    pinMode(CALIBRATION_LED_PIN, OUTPUT);
    pinMode(FIRST_MPU_WARNING_LED_PIN, OUTPUT);
...
void loop()
{
                         ...
                         that the collected data is reliable */
    {
        ...
        else
        {
            ...
            secondCANTime = millis();
            thirdCANTime = millis();
        }
        servoAngle = RESTING_POSITION; // while calibrating, the servomotor has to stay at its resting position
    }
    else
    {
        ...
        {
            sensors_event_t a, g, temp;
            if(firstMPUWorking) // If the MPU was previously marked as working, try to read data
            {
                ...
                if (i2cError == 0)
                {
                    firstMPUReading = firstMPU.getEvent(&a, &g, &temp); // Connection is ok, getting the data from the MPU
                }
                else
                {
            ...
            else
            {
                ...
                if(currentClockTime - lastRestartingTime >= RESTARTING_TIME)
                {
                    ...
                    if(i2cError == 0)
                    {
                        if(firstMPU.begin()) // If the sensor responds, it has to re-initialize the library
                        {
            ...
            if(firstMPUWorking && firstMPUReading)
            {
                /* If the I2C communication is fully working, 
                   save the reading in the structure */ 
                firstCAN_data.readingValueTX = (a.acceleration.x - offset);
            }
            else
            {
                firstCAN_data.readingValueTX = 999.0f; // If the MPU isn't working, send 999 as data
            }
            dataArray[0] = firstCAN_data.readingValueTX; // Setting the array's first value 
                                                                      ...
                                                                      I had to make this choice, otherwise the busses would've overflowed */
            {
                ...
                txMsg.can_id = FIRST_ID; // Setting the sender ID
                ...
                {
                    txMsg.data[iterations1] = firstCAN_data.readingValueBytesTX[iterations1];
                }
                firstMCP.sendMessage(&txMsg);
            }
                                                                     ...
                                                                     the program from freezing under certain conditions */
            {
                ...
                interrupts();
                while(firstMCP.readMessage(&rxMsg) == MCP2515::ERROR_OK)
                {
                    if(rxMsg.can_id == SECOND_ID) // Checking if the relevated id matches 0x200
                    {
                        /* Deserialize the incoming CAN payload, reconstruct the float value 
                           by copying the bytes into the union structure */
                        for(iterations1 = 0; iterations1 < DATA_LENGTH; iterations1++)
                        {
                            secondCAN_data.readingValueBytesRX[iterations1] = rxMsg.data[iterations1]; // Transfering the bytes to a local variable
                        }
                        secondCANTime = currentClockTime;
                        if(secondCAN_data.readingValueRX == 999.0f)
                        {
                        ...
                        else
                        {
                            ...
                            dataArray[1] = secondCAN_data.readingValueRX;
                        }
                    }
            ...
            /* The second node failed to send a new message within the 
               time limit, mark it as not working */
            if(currentClockTime - secondCANTime >= MCP_CONTROL_TIME)
            {
                secondMPUWorking = false;
            ...
            if(secondMPUWorking && thirdMPUWorking)
            {
            ...
            else
            {
                ...
                {
                    inclination = (firstCAN_data.readingValueTX + secondCAN_data.readingValueRX) / 2.00f;
                }
                else if(firstMPUWorking && !secondMPUWorking && thirdMPUWorking)
                {
                    inclination = (firstCAN_data.readingValueTX + thirdCAN_data.readingValueRX) / 2.00f;
                }
                else if(!firstMPUWorking && secondMPUWorking && thirdMPUWorking)
                {
                    inclination = (secondCAN_data.readingValueRX + thirdCAN_data.readingValueRX) / 2.00f;
                }
                /*                      === Second Case ===
                ...
                {
                    inclination = firstCAN_data.readingValueTX;
                }
                else if(!firstMPUWorking && secondMPUWorking && !thirdMPUWorking)
                {
                    inclination = secondCAN_data.readingValueRX;
                }
                else if(!firstMPUWorking && !secondMPUWorking && thirdMPUWorking)
                {
                ...
                servoAngle = map((long)(inclination * 1000), -10000, 10000, 0, 180);
    ...
    digitalWrite(FIRST_MPU_WARNING_LED_PIN, warningLed);
