...
#define THIRD_MPU_WARNING_LED_PIN 32
...
MCP2515 thirdMCP(CS_PIN);// Creating an instance for the can bus module
...
Receiver firstCAN_data, secondCAN_data;

Adafruit_MPU6050 thirdMPU;// Creating an instance for the mpu6050 sensor

Servo thirdServo;// Creating an istance for the servomotor
...
bool thirdMPUWorking = false, thirdMPUReading = false, warningLed = LOW;
long unsigned int lastRestartingTime = 0;
// Variables made for the fault tolerance system regarding the other ESP32s
long unsigned int firstCANTime = 0, secondCANTime = 0;
bool firstMPUWorking = false, secondMPUWorking = false;
...
void setup()
{
    ...
    thirdMCP.reset();
    thirdMCP.setBitrate(CAN_125KBPS, MCP_8MHZ);
    thirdMCP.setNormalMode();
    ...
    {
        thirdMPUWorking = thirdMPU.begin(); /* If the I2C communication is successful, 
                                        try to initialize the sensor library */
    } 
    else 
    {
        thirdMPUWorking = false; // If the I2C transmission failed, mark the local MPU as not working
    }
    if(!thirdMPUWorking) // Handling the fault tolerance
    {
    ...
    thirdServo.attach(SERVO_PIN);
    thirdServo.setPeriodHertz(50);

    pinMode(BUTTON_PIN, INPUT);
    pinMode(ONOFF_LED_PIN, OUTPUT);
    pinMode(CALIBRATION_LED_PIN, OUTPUT);
    pinMode(THIRD_MPU_WARNING_LED_PIN, OUTPUT);
...
void loop()
{
    ...
    else
    {
        ...
        {
            sensors_event_t a, g, temp;
            if(thirdMPUWorking) // If the MPU was previously marked as working, try to read data
            {
                ...
                if (i2cError == 0)
                {
                    thirdMPUReading = thirdMPU.getEvent(&a, &g, &temp); // Connection is ok, getting the data from the MPU
                }
                else
                {
                    // I2C bus error detected: trigger the fault recovery sequence
                    thirdMPUReading = false;
                    warningLed = HIGH;   
            ...
            else
            {
                ...
                if(currentClockTime - lastRestartingTime >= RESTARTING_TIME)
                {
                    ...
                    if(i2cError == 0)
                    {
                        if(thirdMPU.begin()) // If the sensor responds, it has to re-initialize the library
                        {
                            thirdMPUWorking = true;
                            warningLed = LOW; // Turning off the warning LED as the fault is resolved
                            thirdMPU.getEvent(&a, &g, &temp);
                            thirdMPUReading = true;
                        }
                    }
                    else 
                    {
            ...
            if(thirdMPUWorking && thirdMPUReading)
            {
                /* If the I2C communication is fully working, 
                   save the reading in the structure */ 
                thirdCAN_data.readingValueTX = (a.acceleration.x - offset);
            }
            else
            {
                thirdCAN_data.readingValueTX = 999.0f; // If the MPU isn't working, send 999 as data
            }
            dataArray[2] = thirdCAN_data.readingValueTX; // Setting the array's third value
                                                                      ...
                                                                      I had to make this choice, otherwise the busses would've overflowed */
            {
                ...
                txMsg.can_id = THIRD_ID; // Setting the sender ID
                ...
                {
                    txMsg.data[iterations1] = thirdCAN_data.readingValueBytesTX[iterations1];
                }
                thirdMCP.sendMessage(&txMsg);
            }
                                                                     ...
                                                                     the program from freezing under certain conditions */
            {
                ...
                interrupts();
                while(thirdMCP.readMessage(&rxMsg) == MCP2515::ERROR_OK)
                {
            ...
            if(firstMPUWorking && secondMPUWorking)
            {
            ...
            else
            {
                ...
                {
                    inclination = (firstCAN_data.readingValueRX + secondCAN_data.readingValueRX) / 2.00f;
                }
                else if(firstMPUWorking && !secondMPUWorking && thirdMPUWorking)
                {
                    inclination = (firstCAN_data.readingValueRX + thirdCAN_data.readingValueTX) / 2.00f;
                }
                else if(!firstMPUWorking && secondMPUWorking && thirdMPUWorking)
                {
                    inclination = (secondCAN_data.readingValueRX + thirdCAN_data.readingValueTX) / 2.00f;
                }
                /*                      === Second Case ===
                ...
                {
                    inclination = thirdCAN_data.readingValueTX;
                }
